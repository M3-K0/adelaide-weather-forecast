#!/usr/bin/env python3
"""
ERA5 Yearly Data Downloader for Adelaide Weather Forecasting
Downloads ERA5 data in yearly chunks to minimize queue wait times.
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('era5_yearly_download.log'),
...
def download_era5_year(client, year, variables, area, output_dir, dataset_type='pressure_levels'):
    """Download ERA5 data for a full year with progress monitoring."""
    ...
    if dataset_type == 'pressure_levels':
        ...
        filename = f"era5_surface_{year}.nc"
    ...
    # Prepare request parameters for full year
    request_params = {
    ...
    print(f"   📅 Full year: 12 months, ~365 days")
    print(f"   📍 Area: Adelaide region")
    print(f"   ⚠️  Large request - may take time to process in CDS queue")
    ...
    try:
        ...
        print(f"🕐 Queue status check: https://cds.climate.copernicus.eu/live/queue")
        ...
        # Submit the request (this will wait in CDS queue)
        client.retrieve(
            dataset_name,
            request_params,
            str(output_path)
...
def convert_to_zarr(netcdf_files, output_zarr, dataset_type='pressure_levels'):
    """Convert yearly NetCDF files to a single Zarr dataset."""
...
def download_era5_dataset(start_year=2010, end_year=2020):
    """Download complete ERA5 dataset in yearly chunks."""
    
    logger = setup_logging()
    logger.info("Starting ERA5 yearly download for Adelaide weather forecasting")
    ...
    total_files = total_years * 2  # pressure + surface
    
    # Overall progress tracking
    print(f"\n🚀 Starting ERA5 Yearly Download Campaign")
    print(f"📅 Period: {start_year}-{end_year} ({total_years} years)")
    print(f"📍 Region: Adelaide, Australia")
    print(f"💾 Output: {zarr_dir}")
    print(f"⚡ Strategy: Yearly chunks to minimize queue waits")
    ...
    for year in range(start_year, end_year + 1):
        ...
        try:
            ...
            pressure_file = download_era5_year(
                client, year, variables, area, 
            ...
            print("\n🔄 Surface dataset:")
            surface_file = download_era5_year(
                client, year, variables, area,
                surface_dir, 'surface'
            )
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("ERA5 Yearly Download Summary")
    logger.info("="*60)
    ...
    logger.info("ERA5 yearly download process completed")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download ERA5 data in yearly chunks')
    parser.add_argument('--start-year', type=int, default=2010,
                       help='Start year (default: 2010)')
    parser.add_argument('--end-year', type=int, default=2020,
                       help='End year (default: 2020)')
    parser.add_argument('--test-year', action='store_true',
                       help='Download only 2020 for testing')
    
    args = parser.parse_args()
    
    if args.test_year:
        # Download just 2020 for testing
        logger = setup_logging()
        logger.info("Test mode: downloading 2020 only")
