#!/usr/bin/env python3
"""
ERA5 Monthly Data Downloader for Adelaide Weather Forecasting
Downloads ERA5 data in monthly chunks to avoid CDS API size limits.
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('era5_monthly_download.log'),
...
def download_era5_month(client, year, month, variables, area, output_dir, dataset_type='pressure_levels'):
    """Download ERA5 data for a single month with progress monitoring."""
    ...
    if dataset_type == 'pressure_levels':
        filename = f"era5_pressure_{year}_{month:02d}.nc"
        dataset_name = 'reanalysis-era5-pressure-levels'
    else:
        filename = f"era5_surface_{year}_{month:02d}.nc"
    ...
    print(f"   📅 Days: {len(request_params['day'])}")
    ...
    try:
        ...
        # Submit the request (no progress callback - CDS API doesn't support it)
        client.retrieve(
            dataset_name,
            request_params,
            str(output_path)
...
def convert_to_zarr(netcdf_files, output_zarr, dataset_type='pressure_levels'):
    """Convert monthly NetCDF files to a single Zarr dataset."""
...
def download_era5_dataset(start_year=2010, end_year=2020):
    """Download complete ERA5 dataset in monthly chunks."""
    
    logger = setup_logging()
    logger.info("Starting ERA5 monthly download for Adelaide weather forecasting")
    ...
    print(f"\n🚀 Starting ERA5 Download Campaign")
    print(f"📅 Period: {start_year}-{end_year} ({total_months} months)")
    ...
    overall_pbar = tqdm(
        total=total_months * 2,  # *2 for pressure + surface
    ...
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            ...
            print(f"\n📆 {year}-{month:02d} ({month_name}) - Month {current_month}/{total_months}")
            print("-" * 50)
            
            try:
                # Download pressure levels
                print("🔄 Pressure levels dataset:")
                pressure_file = download_era5_month(
                    client, year, month, variables, area, 
                ...
                print("\n🔄 Surface dataset:")
                surface_file = download_era5_month(
                    client, year, month, variables, area,
                    surface_dir, 'surface'
                )
                ...
                remaining_files = (total_months * 2) - completed_files
                ...
                if completed_files > 0:
                    ...
                    print(f"\n📈 Campaign Statistics:")
                    print(f"   ✅ Completed: {completed_files}/{total_months * 2} files")
                    print(f"   ⏱️  Elapsed: {elapsed_time/3600:.1f} hours")
                    print(f"   🕐 ETA: {eta_hours:.1f} hours remaining")
                    print(f"   📊 Progress: {(completed_files/(total_months*2)*100):.1f}%")
    ...
    # Summary
    logger.info("\n" + "="*60)
    logger.info("ERA5 Monthly Download Summary")
    logger.info("="*60)
    ...
    logger.info("ERA5 monthly download process completed")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download ERA5 data in monthly chunks')
    parser.add_argument('--start-year', type=int, default=2010,
                       help='Start year (default: 2010)')
    parser.add_argument('--end-year', type=int, default=2020,
                       help='End year (default: 2020)')
    parser.add_argument('--test-month', action='store_true',
                       help='Download only one month for testing')
    
    args = parser.parse_args()
    
    if args.test_month:
        # Download just January 2020 for testing
        logger = setup_logging()
        logger.info("Test mode: downloading January 2020 only")
